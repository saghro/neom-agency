...
const LinkCard = ({ title, links }) => {
    return (
      <div className="text-white p-12 rounded-lg w-full max-w-7xl mx-auto font-poppins" style={{ background: 'linear-gradient(to right, #A8D9E0, #6A71B3)' }}>
        <h2 className="text-3xl font-bold mb-8 text-center">{title}</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {links.map((link, index) => (
            <a key={index} href={link.url} className="flex items-center justify-between p-6 rounded-full transition duration-300 hover:bg-[#6A71B3]">
              <span>{link.text}</span>
